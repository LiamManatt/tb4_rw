...
class TurtleDodger(Node):

    def __init__(self):
        super().__init__("turtle_dodger")
        ...
        self.timer_ = self.create_timer(0.5, self.drive)
    ...
    def odom_callback(self, msg):
        ...
        if iteration == 1:
            ...
            target[2] = state_arr[0][2]   # here I set the constant target orientation
            print('target:', target)
        ...
        if ((distance_to_obstacle <= obstacle_radius + robot_radius + safety_margin) and (dodge == 0)):
            ...
            #### new controller
        if dodge == 0:
            if np.sqrt((state_arr[0][0]-target[0])**2 + (state_arr[0][1]-target[1])**2) < 0.02 and abs(diff_arr[2]) <= 0.1:    
            ...
            if np.sqrt((state_arr[0][0]-target[0])**2 + (state_arr[0][1]-target[1])**2) <= 0.02 and abs(diff_arr[2]) > 0.05:
                robot_velocity = 0
                spindir = (diff_arr[2])/abs(diff_arr[2])
                angularSpeed = min(0.02, abs((diff_arr[2])/20))
                robot_omega = spindir*angularSpeed

            # if robot isn't near destination, turn robot until it's pointing at current goal coordinates
            if np.sqrt((state_arr[0][0]-target[0])**2 + (state_arr[0][1]-target[1])**2) >= 0.02 and abs(diff_arr[2]) > 0.1:
                scale = 20
                spinway = (diff_arr[2])/(abs(diff_arr[2]))
                angSpeed = min(0.04, abs((diff_arr[2])/scale))
                robot_omega = spinway*angSpeed
                robot_velocity = 0
            
            # if robot is pointing in the right direction and isn't at goal coordinates, move forwards
            if np.sqrt((state_arr[0][0]-target[0])**2 + (state_arr[0][1]-target[1])**2) >= 0.02 and abs(robot_omega) <= 0.01:
                robot_velocity = 0.08
        ...
        if dodge == 1:
            ...
            if doneBezier:
                ...
                if np.sqrt((diff_arr[0])**2 + (diff_arr[1])**2) < 0.02 and abs(diff_arr[0][2]) <= 0.1:    
                    print("Goal destination reached!? \n")
                    # stop the robot when the goal destination is reached (after going through waypoints)
                    robot_velocity = 0
                    robot_omega = 0
                ...
                # if at final goal coordinates, correct the orientation if needed
                if np.sqrt((diff_arr[0])**2 + (diff_arr[1])**2) <= 0.02 and abs(diff_arr[2]) > 0.05:
                    robot_velocity = 0
                    spindir = (diff_arr[2])/abs(diff_arr[2])
                    angularSpeed = min(0.02, abs((diff_arr[2])/20))
                    robot_omega = spindir*angularSpeed
                ...
                # if robot isn't near destination, turn robot until it's pointing at current goal coordinates
                if np.sqrt((diff_arr[0])**2 + (diff_arr[1])**2) >= 0.02 and abs(diff_arr[2]) > 0.1:
                    scale = 20
                    spinway = (diff_arr[2])/(abs(diff_arr[2]))
                    angSpeed = min(0.05, abs((diff_arr[2])/scale))
                    robot_omega = spinway*angSpeed
                    robot_velocity = 0
                
                # if robot is pointing in the right direction and isn't at goal coordinates, move forwards
                if np.sqrt((diff_arr[0])**2 + (diff_arr[1])**2) >= 0.02 and abs(diff_arr[2]) <= 0.01:
                    robot_velocity = 0.08
    ...
    def display_states(self):
        global diff_arr, state_arr, target, robot_velocity, robot_omega
        print("state: ", state_arr[0])
        print("target: ", target)
        print("diff_arr: ", diff_arr)
        print("velocity: ", robot_velocity , " omega: ", robot_omega)
        print()

    def drive(self):
        # keep publishing the data to cmd_vel to drive the robot
        global robot_velocity, robot_omega, diff_arr, doneBezier
        msg = Twist()
        if dodge == 1 and doneBezier == False:
            robot_velocity = 0
            robot_omega = 0
...
def main(args=None):
    ...
    Odom_data_subscriber = TurtleDodger()
